...
export function ProfileSidebar({ onClose }) {
  ...
  const [refreshingOrders, setRefreshingOrders] = useState(false); // New state for background refresh
  ...
  const fetchOrders = async (isBackground = false) => {
    ...
    try {
      ...
      if (sessionError || !session?.access_token) {
        ...
        else setRefreshingOrders(false);
        return;
      }
  ...
  // Auto-refresh orders every 30 seconds when order history is visible
  useEffect(() => {
    if (showOrderHistory) {
      fetchOrders(); // Initial fetch
      const interval = setInterval(() => fetchOrders(true), 30000); // 30 seconds background refresh
  ...
  const handleOrderHistoryClick = () => {
    ...
    setError("");
    // fetchOrders removed here, handled by useEffect
  };
